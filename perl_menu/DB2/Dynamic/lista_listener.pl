...
#
# PDL1_DYNAMIC  <TAG> STATUS MENU ORACLE LISTENER <TAG>  lista_listener.pl <TAG>
#
# by static entrys, 
# ==============================================================================
...
while (defined ($linea = <INPUT>)) {
      chomp($linea);
      $linea=auxtrim($linea);
      if (((substr($linea,0,1)) =~ /^[a-zA-Z]/) || ((substr($linea,0,4)) =~ /\+ASM/))  {                   ## Fix1.11 cleaning blanks and comments
...
########################################################################
                             ...
                             while (defined ($linea = <ENTRA>)) {
                                     ...
                                     if ($linea =~ /SID_LIST_LISTENER_/)        {
                                                         ...
                                                         print FILERES "$cont <TAG>  LISTENER STATUS $ora_listener  (Us. grid/oracle) <TAG> PGM <TAG> listener_status.pl $ora_listener <TAG> \n ";
                                                                                      }              
                                                           else {                
                                                          print FILERES "$cont <TAG>  LISTENER STATUS  $ora_listener  (User Oracle) <TAG> PGM <TAG> listener_status.pl $ora_listener <TAG> \n ";
                                                                }                       # Fin Parche 1.11
                                                           $cont=pop @opciones;
