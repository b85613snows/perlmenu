...
#  
#  Dinamyc perl to build the stop commands for db2 instances. 
############################################################################################
...
#
# PDI2_DYNAMIC  <TAG>  STOP MENU OF AN INSTANCE DB2 LUW  <TAG> parar_insts_db2.pl 
#
# by static entrys, 
sub auxtrim($)
{
...
my @CmdResult=`$Cmd`;
    ...
    if ($rc eq 0){
             ...
             foreach $_ (@CmdResult) {
                             ...
                             print "$_ \n";
                              if  ( getpwuid( $< ) eq 'root') {
                               print FILERES "$cont <TAG>  STOP INSTANCE (User/$instan) <TAG> PGM <TAG> db2luw stop $instan <TAG> \n ";                            
                                                              }
                                    else                    {
                                print FILERES "$cont <TAG> STOP INSTANCE  (User/$instan) <TAG> PGM <TAG>  db2stop.pl <TAG> \n ";
                                                            }
