#!/usr/bin/perl
###########################################################################################
#  Script: para_insts_ora.pl
#  Output: file ./MenuTemp.txt with the result search instances /etc/oratab 
#  
#  Dinamyc perl to build the stop oracle instances for oracle databases. 
############################################################################################
...
#
# PD02_DYNAMIC  <TAG> STOP MENU ORACLE INSTANCES <TAG>  para_insts_ora.pl <TAG>
#
# by static entrys, 
# ==============================================================================
...
$cont=pop @opciones;
             foreach $_ (@arrayinst) {
                     ...
                     #        print "$_ \n";
                             print FILERES "$cont <TAG>  STOP INSTANCE $orainst (User Oracle) <TAG> PGM <TAG> oracle_stop.pl $orainst <TAG> \n ";
                             $cont=pop @opciones;
                                     } # END foreach
close FILERES;
exit 0
