...
#
# PDI4_DYNAMIC  <TAG>  RESTART OF AN INSTANCE DB2 LUW  <TAG> restart_insts_db2.pl
#
# by static entrys, 
sub auxtrim($)
{
...
my @CmdResult=`$Cmd`;
    ...
    if ($rc eq 0){
             ...
             foreach $_ (@CmdResult) {
                             ...
                             print "$_ \n";
                             if  ( getpwuid( $< ) eq 'root') {
                                print FILERES "$cont <TAG>  RESTART INSTANCE  (User/$instan) <TAG> PGM <TAG> db2luw restart $instan <TAG> \n ";
                                                            }
                                    else                    {
                                print FILERES "$cont <TAG>  RESTART INSTANCE  (User/$instan) <TAG> PGM <TAG>  db2restart.pl <TAG> \n ";
                                                            } 
                             $cont++;
                                     } # END foreach
                 }
