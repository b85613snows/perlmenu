#!/usr/bin/perl
###########################################################################################
#  Script: arranca_grid_instance.pl
...
#
# PGI2_DYNAMIC  <TAG> MENU START DATABASES BY GRID  <TAG> arranca_grid_instance.pl <TAG>
#
# by static entrys, 
# ==============================================================================
sub auxtrim($)
...
if ( $esgrid eq 'no' ) {             ## there in no grid in /etc/oratab
                     open(FILERES, "> $menuTemp") or die "Couldn't open File $menuTemp   \n";
                     print FILERES " \n ";
                     close FILERES;
                    exit 0;
...
my $Cmd="srvctl config database";
   @arrayinst=`$Cmd`;
             foreach $_ (@arrayinst) {
                             ...
                             $orainst=auxtrim($_);
                             $cont=pop @opciones;
                              print FILERES "$cont <TAG>  START DATABASE  $orainst BY GRID  (Us. oracle) <TAG> PGM <TAG> oracle_start_grid_instance.pl $orainst <TAG> \n ";
                             $cont=pop @opciones;
                                     } # END foreach
close FILERES;
exit 0
