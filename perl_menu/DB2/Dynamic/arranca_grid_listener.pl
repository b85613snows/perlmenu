...
#  
#  Dinamyc perl to build the start listener returning by srvctl status listener command. 
############################################################################################
...
#
# PGI4_DYNAMIC  <TAG> MENU START LISTENER BY GRID   <TAG> arranca_grid_listener.pl <TAG>
#
# by static entrys, 
# ==============================================================================
sub auxtrim($)
...
my $auxiliar="";
   @arrayinst=`$Cmd`;
             foreach $_ (@arrayinst) {
                             ...
                             $cont=pop @opciones;
                              print FILERES "$cont <TAG>  START LISTENER  $orainst BY GRID (Us. oracle) <TAG> PGM <TAG> oracle_start_grid_listener.pl $orainst <TAG> \n ";
                             $cont=pop @opciones;
                                   } 
                                     } # END foreach
close FILERES;
