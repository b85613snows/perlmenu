...
while (defined ($linea = <INPUT>)) {
      chomp($linea);
      $linea=auxtrim($linea);
      if (((substr($linea,0,1)) =~ /^[a-zA-Z]/) || ((substr($linea,0,4)) =~ /\+ASM/))  {                   ## Cleaning comment and blanks
...
close INPUT;
   ...
   $cmd="lsnrctl stop LISTENER_$ora_listen";
   @ccmd=`$cmd`;
     foreach $_ (@ccmd) {
                         print "$_ ";
                         } # END foreach  
